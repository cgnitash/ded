    ...
    linear::initialize()
{
  ...
  if (load_spec_.empty())
  {
    ...
    ranges::generate_n(ranges::back_inserter(pop_), size_, [&] {
      auto org = life::make_entity(org_);
  ...
  } else
  {
    ...
    {
      std::cout << "error: path \"" << life::global_path + load_spec_
                << "\" does not exist";
      std::exit(1);
    }
    ...
    linear::prune_lineage(long i)
{
  ...
  auto lineage_file = util::open_or_append(
      life::global_path + "lineage.csv", "id,recorded_at,on_lod,ancestor_id\n");
