                        ...
                        std::string return_column) const
{
  ...
  auto const value_count = std::count_if(
      std::begin(rows_), std::end(rows_), [value, column_index](auto &row) {
...
void CSV::merge(CSV merge_csv, std::string column)
{
  ...
  if (!std::includes(std::begin(merge_values),
                     std::end(merge_values),
                     std::begin(lookup_values),
                     std::end(lookup_values)))
  {
  ...
  for (auto const &merge_column : merge_csv.column_names())
  {
    ...
    if (std::find(std::begin(column_names_),
                  std::end(column_names_),
