...
public:
  ...
  specs::PopulationSpec publish_configuration()
  {
    return self_->publish_configuration_();
  }

  void configure(specs::PopulationSpec es)
  {
    self_->configure_(es);
  }
...
private:
  ...
  struct PopulationObject final : PopulationInterface
  {
    ...
    PopulationInterface *copy_() const override
    {
	...
	specs::PopulationSpec publish_configuration_() override
    {
    ...
    void configure_(specs::PopulationSpec c) override { data_.configure(c); }
